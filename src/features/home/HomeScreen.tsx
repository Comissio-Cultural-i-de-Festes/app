...
/** "En Marc, la Júlia i 3 més s'han apuntat avui" — two names, then a count. */
function movementLine(names: readonly string[], t: Translate): string {
  ...
  // names are separated by a comma, or the line reads "Bravo i Echo i 2 més".
  const people = shown.join(others > 0 ? t('home.movement.comma') : t('home.movement.and'))

  return others > 0
    ? t('home.movement.more', { people, count: others })
    : t('home.movement.only', { people, count: shown.length })
}
