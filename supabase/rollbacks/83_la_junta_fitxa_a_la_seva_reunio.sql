-- Rollback de la migració 83. NO és a `migrations/` a posta: un fitxer aquí
-- dins el desfaria el mateix `db push` que acaba d'aplicar-lo.
--
-- Torna `check_in_here` al cos de la 65 sencer: cap reunió es torna a poder
-- fitxar des del lloc, ni tan sols per qui hi és convocat.
--
-- QUÈ TORNA A OBRIR. La junta, a la sala de la seva reunió, tornarà a prémer
-- «Sóc aquí» i a llegir que l'esdeveniment no existeix. Les assercions noves de
-- `supabase/tests/340_reunions.test.sql` es posaran vermelles, que és el que han
-- de fer. Les files d'`attendances` fitxades mentrestant es queden.
