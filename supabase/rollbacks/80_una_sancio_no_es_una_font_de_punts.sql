...
--
-- ── ORDRE: LA 82 VA PRIMER ──────────────────────────────────────────────────
--
-- El cos d'aquí és el de la 48 i la 82 va tornar a escriure `admin_dashboard`
-- per tancar el que la 80 deixava obert: un avís retirat el curs següent, que
-- amb l'agrupació sola torna a sortir com una font de punts a la finestra de la
-- retirada. Aplicat sobre una base amb la 82, aquest fitxer la desfà en silenci
-- —`create or replace` no avisa de res— i el forat torna sencer.
--
-- `supabase/rollbacks/82_...sql` existeix i desfà només la 82. L'ordre és 82 i
-- després aquest; al revés, el segon desfaria el primer.
--
-- LA PANTALLA NO ES DESFÀ DES D'AQUÍ, i el que hi queda ja no és el que aquest
-- paràgraf deia abans. `DashboardScreen.tsx` treu del gràfic els motius que no
-- sumen, i des de la revisió de la 82 no ho decideix pel signe: `avis` i
-- `avis_retirat` no són mai una font de punts, tinguin el número que tinguin
-- (`dashboardPoints.ts`). Amb aquest fitxer aplicat, doncs, la llista torna a
-- portar `avis_retirat` com a motiu propi i positiu, i el client el posa a la
-- línia de text de sota en comptes de dibuixar-lo com una barra. La targeta no
-- queda trencada ni menteix; el que passa és que el panell torna a no ensenyar
-- els avisos que es mantenen ni els ajustos a mà que resten, i torna a explicar
-- un curs diferent del que explica el perfil del soci. El desfer de debò és
-- també revertir el commit del client.
