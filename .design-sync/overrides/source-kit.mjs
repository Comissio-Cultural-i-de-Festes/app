// forked from design-sync lib/source-kit.mjs — app repo, not a component library: src paths are feature folders, so grouping comes from .design-sync/docs frontmatter instead
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // FORK: no group is derived from the source path here.
      //
      // Upstream takes the last meaningful directory segment. That works for a
      // component library, where the folder a component lives in IS its group.
      // app-comi is an application: these files sit in `src/ui/<Name>/` and in
      // `src/features/<feature>/`, so the derivation yields a group per feature
      // folder — thirteen of them for twenty-one components, most with a single
      // member (share, rides, junta, logo, tabbar…), and it silently outranks
      // the `category` frontmatter in .design-sync/docs/. Leaving it 'general'
      // is what lets those categories decide, which is where the grouping is
      // actually curated.
      c.group = 'general';
    }
  }
